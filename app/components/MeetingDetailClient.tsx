...
export default function MeetingDetailClient({ id }: { id: string }) {
  ...
  // Instant render: If seed data exists, initialize state directly
  const [meeting, setMeeting] = useState<Meeting | null>(seedMeetings[id] || null);
  const [loading, setLoading] = useState(seedMeetings[id] ? false : true);
  ...
  // Fetch Member role details from members table based on email prefix
  useEffect(() => {
    if (!user) {
      setMemberRole("멤버");
      return;
    ...
    const fetchMemberRole = async () => {
      try {
        ...
        console.warn("Failed to fetch member role from DB:", e);
      }
    };
  ...
  // Fetch meeting data in the background (Non-blocking RAG)
  useEffect(() => {
    if (!id) return;
    ...
    const fetchMeetingInfo = async () => {
      try {
        ...
        console.warn("Background fetch failed, keeping local seed/cache state:", err);
      } finally {
        setLoading(false);
      }
    };

    // If no seed meeting is pre-loaded, trigger loading spinner
    if (!meeting) {
      setLoading(true);
    }
    fetchMeetingInfo();
  ...
  if (!meeting) {
    return (
      <>
        <Navbar />
        <div className="min-h-screen bg-[#BDF1E7] flex flex-col items-center justify-center pt-32 text-black animate-fadeIn">
          <span className="text-5xl mb-4">🔍</span>
          <h2 className="text-xl font-bold font-serif mb-2">모임을 찾을 수 없습니다</h2>
          <button onClick={() => router.push("/")} className="px-4 py-2.5 bg-black text-[#BDF1E7] rounded-xl font-bold text-xs hover:bg-[#203D39] transition-colors cursor-pointer">
            메인 페이지로 이동
          </button>
        </div>
        <Footer />
  ...
  // Authorization Check:
  // Current user is recognized as leader if:
  // 1. user.id equals meeting.leader_id
  // 2. meeting.leader_id is "guest"
  // 3. User's role in the members table is "모임장" or "운영진"
  const isLeader = user && (
    user.id === meeting.leader_id || 
    meeting.leader_id === "guest" || 
    memberRole === "모임장" ||
    memberRole === "운영진"
  );
  ...
  const handleJoinToggle = () => {
    if (!user) {
      ...
      if (loginBtn) {
        ...
        alert("상단 바의 [로그인] 버튼을 클릭하여 로그인해 주세요.");
      }
      return;
    }
    ...
    alert(nextState ? "🌿 모임 참석 신청이 완료되었습니다! 일정에 맞춰 참여해 주세요." : "참석 신청이 취소되었습니다.");
  };
      ...
      <main className="min-h-screen bg-[#BDF1E7] pt-32 pb-24 px-8 text-[#142825] animate-fadeIn">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-6 border-t border-black/10">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-extrabold mt-0.5">{meeting.location || "장소 미정"}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <span className="text-xl">👥</span>
                <div>
                  <h4 className="text-[10px] font-extrabold text-black/40 uppercase tracking-wider">참가 인원</h4>
                  <p className="text-xs font-extrabold mt-0.5">{meeting.attendees} / {meeting.max_attendees} 명</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {/* LEFT COLUMN: announcements & details */}
            <div className="lg:col-span-7 space-y-8">
              ...
              <div className="p-6 md:p-8 border border-black/15 bg-white/45 backdrop-blur-md rounded-2xl">
                <div className="flex items-center justify-between mb-6 pb-3 border-b border-black/10">
                  ...
                  {isLeader && (
                    <span className="text-[9px] font-extrabold bg-[#142825] text-[#BDF1E7] px-2.5 py-1 rounded-md tracking-wider uppercase">
                      모임장 권한 활성
                    </span>
                  )}
                </div>

                {/* Announcement Writer Form (Only visible to the leader) */}
                {isLeader && (
                  <form onSubmit={handlePublishAnnouncement} className="mb-8 p-5 bg-white/60 border border-black/10 rounded-xl animate-fadeIn">
                    <label className="block text-[9px] font-bold text-black/50 uppercase tracking-widest mb-2">
                      🔒 새로운 공지 또는 발제 토론 주제 등록
                    </label>
                    <textarea
                      required
                      rows={3}
                      ...
                      placeholder="멤버들에게 전달할 공지사항이나 사전 독서 발제문을 적어주세요..."
                    ...
                    <div className="flex justify-end">
                      <button
                        type="submit"
                        className="px-4 py-2 bg-black text-[#BDF1E7] rounded-xl text-xs font-bold hover:bg-[#203D39] transition-all cursor-pointer"
                      >
                        공지 등록하기
                      </button>
                    </div>
                ...
                {announcements.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {announcements.map((ann, idx) => (
                      <div key={ann.id || idx} className="p-5 bg-white/60 border border-black/10 rounded-xl relative hover:border-black/25 transition-all">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-xs">📢</span>
                          <span className="text-[10px] font-extrabold text-[#142825]/70">
                            {idx === announcements.length - 1 ? "최초 모임 개설 안내 공지" : "추가 공지"}
                          </span>
            ...
            <div className="lg:col-span-5 space-y-8">
              ...
              <div className="p-6 md:p-8 border border-black/15 bg-white/35 backdrop-blur-md rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.02)]">
                ...
                {isLeader ? (
                  <div className="p-4 border-2 border-black bg-white/70 rounded-xl text-center">
                    <span className="text-xl block mb-1">👑</span>
                    <h4 className="text-xs font-extrabold text-black uppercase tracking-wider">모임장 전용 메뉴</h4>
                    <p className="text-[10px] text-black/60 font-semibold leading-relaxed mt-1">
                      당신은 이 모임의 관리자입니다. 왼쪽 메뉴에서 멤버들에게 안내할 새로운 공지나 발제 토론 주제를 작성해 보세요.
                    </p>
                  </div>
                ) : (
                  <button
                    onClick={handleJoinToggle}
                    className={`w-full py-4 rounded-xl text-xs font-extrabold tracking-widest uppercase transition-all duration-300 active:scale-[0.98] cursor-pointer ${
                      isJoined 
