...
export async function POST(req: Request) {
  try {
    ...
    // Graceful fallback helper if Gemini is not configured or fails
    const runFallbackKeywordRouting = (query: string) => {
    ...
    if (!apiKey || apiKey === "your_gemini_api_key_here") {
      console.warn("GEMINI_API_KEY is not configured. Falling back to keyword-based mock routing.");
      const result = runFallbackKeywordRouting(userMessage);
          ...
          responseSchema: {
            ...
            properties: {
              selected_quote_index: {
                type: "INTEGER" as any,
                description: "The 0-based index of the selected quote from the provided quotes list that best fits the user's situation.",
              },
              aiAdvice: {
                type: "STRING" as any,
                description: "A deep, empathetic, and beautifully written poetic essay/advice in Korean, connecting the selected quote to the user's situation.",
              },
            },
            required: ["selected_quote_index", "aiAdvice"],
          },
        },
      });

      // Format candidate quotes for prompt
      const formattedQuotes = quotesToSearch.map((q, idx) => {
        return `[Index ${idx}]
- Book: ${q.book_title}
- Quote: "${q.quote_text}"
- User Note: ${q.user_note || "None"}`;
      }).join("\n\n");

      const prompt = `You are 'Boreum Companion', a literary and empathetic AI oracle for 'Boreum' (meaning Full Moon in Korean), a youth-centered STS (Science, Technology, and Society) reading community. 
Your goal is to offer deep, comforting, and intellectual guidance to a reader who is sharing their worries.

Here is the user's current situation or worry:
"${userMessage}"

Below are the candidate literary quotes saved in the user's personal scrapbook (wisdom archive). Choose exactly ONE quote from this list that best addresses or comforts their situation:
${formattedQuotes}

Instructions:
1. Identify the single best-fitting quote from the candidate list above. Return its index as 'selected_quote_index'.
2. Write a deep, empathetic, and highly literary poetic essay/advice in Korean (한국어) as 'aiAdvice'.
   - Keep the tone warm, intellectual, and deeply comforting, reflecting the vibe of a thoughtful reading companion.
   - Ground the advice in the philosophy of the selected quote.
   - Gently weave in the user's note/thoughts if appropriate to show personalized attention.
   - The advice should be composed of 3 to 5 readable paragraphs, separated by newlines.
   - Avoid generic, robotic advice or typical assistant prefixes. Start directly with a warm observation.
   - Address the reader with respectful and friendly Korean ("해요체" or "하십시오체").`;

      const result = await model.generateContent(prompt);
      const text = result.response.text();
      const parsed = JSON.parse(text);

      const selectedIdx = typeof parsed.selected_quote_index === 'number' ? parsed.selected_quote_index : 0;
      // Clamp index to safe range
      const finalIdx = Math.max(0, Math.min(selectedIdx, quotesToSearch.length - 1));
      const selectedQuote = quotesToSearch[finalIdx];

      return NextResponse.json({
        book_title: selectedQuote.book_title,
        quote_text: selectedQuote.quote_text,
        user_note: selectedQuote.user_note || "내가 직접 마음에 들어 기록해두었던 가치 있는 단서.",
        aiAdvice: parsed.aiAdvice,
        isFallback: false
      });
    } catch (apiError) {
      console.error("Gemini API call failed, running keyword fallback:", apiError);
      const result = runFallbackKeywordRouting(userMessage);
      return NextResponse.json(result);
    }
  } catch (error) {
